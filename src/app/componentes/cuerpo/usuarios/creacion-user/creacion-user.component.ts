...
import { ListadosUserComponent } from '../listados-user/listados-user.component';


@Component({
  selector: 'app-creacion-user',
  templateUrl: './creacion-user.component.html',
  styleUrls: ['./creacion-user.component.css']
})
export class CreacionUserComponent {
...
// acta_reserva_2
    ...
    const nombre = inf_usurio.value.nombre
    const usuario = inf_usurio.value.usuario
    const contrasenia = inf_usurio.value.contrasenia
    const roll = inf_usurio.value.roll
    const tipo_unidad = inf_usurio.value.tipo_unidad
    const unidad = inf_usurio.value.unidad
    const unidad_dependencia = inf_usurio.value.unidad_dependencia

    

    this.validar_informacion(nombre, "nombre")
    this.validar_informacion(usuario, "usuario")
    this.validar_informacion(contrasenia, "contrasenia")
    this.validar_informacion(roll, "roll")
    this.validar_informacion(tipo_unidad, "tipo_unidad")
    this.validar_informacion(unidad, "unidad")
    this.validar_informacion(unidad_dependencia, "unidad_dependencia")
    ...
    const excel_anios = inf_usurio.value.excel_anios
    const seguimineto_plazos_respueta = inf_usurio.value.seguimineto_plazos_respueta

    const insitop_per = inf_usurio.value.insitop
    const cargue_insitop = inf_usurio.value.ingreso_insitop
    const estadistica_insitop = inf_usurio.value.reporte_insitop
  ...
  const obj_4 = inf_usurio.value.obj_4
    ...
    this.api_serve.conexion_api_enpoint(this.formData, ":5200", "/api/user").subscribe({
