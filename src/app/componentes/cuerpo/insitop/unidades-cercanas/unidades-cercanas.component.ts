...
@Component({
  selector: 'app-unidades-cercanas',
  templateUrl: './unidades-cercanas.component.html',
  styleUrls: ['./unidades-cercanas.component.css']
})
export class UnidadesCercanasComponent {
  form: FormGroup;
  aisladas: any[] = [];
  filtradas: any[] = [];
  filtros: Filtros = {
  ...
  loading = false;
  apiUrl = 'http://172.22.2.36:5010/unidades_cercanas';
...
exportarExcel(): void {
  const datosExportar = this.filtradas.map(u => ({
    ...
    'Unidad Cercana': u.unidad_cercana,
    'Compañía Cercana': u.compania_cercana,
    'Pelotón Cercano': u.peloton_cercana,
    'Distancia mínima (m)': u.distancia_minima
  }));

  const hoja = XLSX.utils.json_to_sheet(datosExportar);
  const libro = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(libro, hoja, 'Unidades Cercanas');
  ...
  const nombreArchivo = `unidades_cercanas_${yyyy}${mm}${dd}.xlsx`;
