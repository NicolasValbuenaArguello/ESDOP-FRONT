...
@Component({
  selector: 'app-registro-q5',
  templateUrl: './registro-q5.component.html',
  styleUrls: ['./registro-q5.component.css']
})
export class RegistroQ5Component {
...
cargar_resultado(form: NgForm){
  ...
  this.dato.push(grado)
  this.dato.push(nombre)
...
guardar_evento(form: NgForm) {
  // const numero_boletin_div = form.value.numero_boletin_div
    ...
    const tipo_fuerza = form.value.tipo_fuerza
    const resultado = form.value.resultado
  ...
  this.validar_informacion(tipo_fuerza, "tipo_fuerza")
  this.validar_informacion(divi_padre, "divi_padre")

  this.formData.append("cp", cp);
  this.formData.append("pel", pel);
  this.formData.append("divi_hija", divi_hija);
  this.formData.append("brigada", brigada);
  this.formData.append("batallon", batallon);
  this.formData.append("sitio", sitio)
  this.formData.append("estructura", estructura)

  this.formData.append("hecho_nuevo", hecho_nuevo)
  this.formData.append("hecho", hecho)
  this.formData.append("resultado", resultado)
  ...
  if (!this.validacion) {
    ...
    this.api_serve.cargar_afectaciones_q5(this.formData).subscribe({
